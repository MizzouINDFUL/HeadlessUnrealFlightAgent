...
private:
  ...
  UPROPERTY()
  TArray<FName> TagsToTrack; //Actors with these tags will be tracked for ground truth

  UFUNCTION()
  FBox2D GetActorBoxFromRT(TArray<FColor> OriginalSurface, UTextureRenderTarget2D *RT) const;

  UPROPERTY()
  FBox2D NextFrameGT;

  UFUNCTION()
  void DrawBox2D(UTextureRenderTarget2D *RT, FBox2D Box, FColor BoxColor) const;

  UPROPERTY()
  UTextureRenderTarget2D* CustomColorRenderTarget;
